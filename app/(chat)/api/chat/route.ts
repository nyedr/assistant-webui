...
// Helper function to fetch available models from the health endpoint
async function getAvailableModel(modelId: string): Promise<Model | null> {
  const baseUrl =
  ...
  try {
    ...
    const availableModel = data.components.models.available.find(
...
export async function POST(request: Request) {
  ...
  const model = await getAvailableModel(modelId);
